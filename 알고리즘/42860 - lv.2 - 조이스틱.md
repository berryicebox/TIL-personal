...
function solution(name) {
    let count = 0
    let move = name.length - 1 // 기본값

    for (let i = 0; i < name.length; i++) {
        // 알파벳 기준
        let up = name[i].charCodeAt(0) - 'A'.charCodeAt(0)
        let down = 'Z'.charCodeAt(0) - 'A'.charCodeAt(0) + 1 - up
        count += Math.min(up, down)
        
        // 이동 기준
        let nextA = i + 1
        while(nextA < name.length && name[nextA] === 'A') nextA ++
        
        let goBack = i * 2 + (name.length - nextA)
        let goForward = (name.length - nextA) * 2 + i 
        
        move = Math.min(move, goBack, goForward)
    }
