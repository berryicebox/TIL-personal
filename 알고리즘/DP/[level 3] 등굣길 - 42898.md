...
function solution(m, n, puddles) {
    ...
    // 웅덩이 좌표 표시
    const blocked = new Set(puddles.map(([x, y]) => `${x},${y}`))
    
    // 연산중인 가로행만 기억하여 이동 횟수 도출
    const dp = Array.from({length: m+1}, () => 0)
    dp[1] = 1
    
    for (let i = 1; i <= n; i++) { // i는 y좌표 위치
        for (let j = 1; j <= m; j++) {
            if (blocked.has(`${j},${i}`)) {
                dp[j] = 0
            } else {
                // 자기 자신 + 이전 값을 더하면 이동 횟수가 나옴
                dp[j] = (dp[j] + dp[j-1]) % MOD // 너무 큰 값이 되지 않도록 MOD로 나눔
            }
        }
    }
