...
function solution(n, edge) {
    // 빈 배열 준비
    const connect = Array.from({ length: n }, () => [])

    // 연결된 노드를 각 노드와 같은 index에 추가
    for (const e of edge) {
    ...
    // 중복되지 않은 경우에만 visited에 추가
    while (queue.length) {
        let cur = queue.shift()
        
        for (const con of connect[cur]) {
            if (!visited[con]) {
                visited[con] = visited[cur] + 1
                queue.push(con)
            }
        }
    }
    ...
    const answer = visited.filter(v => v === max).length
